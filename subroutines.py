#
import os,numpy,random,pandas,copy,scipy.io,scipy.sparse
import matplotlib
matplotlib.use('Agg')
...
#
@numba.jit()
def counts_per_peak(bam_file, peak_bed, out_file, cell_info):
    ...
    outFinal = []
    for barcode in cell_info.index.values:
        if barcode in outData.keys():
            outFinal.append(outData[barcode])
        else:
            outFinal.append([0]*len(peak_info))
...
#
#
def knn_cluster(matrix_df, n_clust, connectivity):
...
#
#
def plot_cluster(cluster_df, n_clust, tsne_result,path,dataname,classname=None):
    seaborn.set(font_scale=2)
    fig3, axes = plt.subplots(1, figsize=(15,15))
    for i in range(0,n_clust):
        index = numpy.where(cluster_df.values==i)[0]
        axes.scatter(tsne_result[index,0], tsne_result[index,1], color=colors[i], label=classname[i])
    axes.legend(bbox_to_anchor=(1, 1), loc=2, borderaxespad=0.)
    fig3.savefig(os.path.join(path,"cluster_{}.tif".format(dataname)), bbox_inches='tight', dpi=600)
    return
#
#
#
...
def specific_accesson(options, subname):
    ...
    peaks_df = pandas.read_csv(options.s + '/matrix/Accesson_peaks.csv', sep='\t', index_col=0,
                   engine='c', na_filter=False, low_memory=False)
    allpeaks_bed = numpy.array(open(options.s+'/peak/top_filtered_peaks.bed').readlines())
...
def specific_peak(options, subname):
    ...
    print(len(cell_inCluster), len(cell_outCluster))
#
    read_inCluster = reads[cell_inCluster, :]
    read_outCluster = reads[cell_outCluster, :]
    mean_in, mean_out = read_inCluster.mean(axis=0).A[0], read_outCluster.mean(axis=0).A[0]
    ttest, pvalues = scipy.stats.ttest_ind(read_inCluster.A, read_outCluster.A, equal_var=False)
    fold = (mean_in + 1e-4) / (mean_out + 1e-4)
    matrix = numpy.vstack((mean_in, mean_out, fold, pvalues)).T
    columns = ['mean_inCluster','mean_outCluster', 'fold', 'p-value']
    ...
    matrix_df = matrix_df.sort_values(by=['p-value'])
#
    peaks_specific = [peaks_bed[int(x[4:])] for x in matrix_df.index.values]
